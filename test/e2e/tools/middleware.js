...
function middlewareFactory(base) {
  ...
  let fixture_regexp = new RegExp('^' + base + '/fixtures/([a-zA-Z0-9_-]+)(/(index.html)?)?$');
  let static_regexp = new RegExp('^' + base + '/fixtures/([a-zA-Z0-9_-]+)(/.*)$');

  return function(req, res, next) {
    let match;
    let basicUrl = req.url;
    let idx = basicUrl.indexOf('?');
    if (idx >= 0) {
    ...
    if ((match = fixture_regexp.exec(basicUrl))) {
      ...
      let rewritten = util.rewriteTestFile(match[1], match[2]);
      if (rewritten !== false) {
        req.url = rewritten;
      }
      next();
