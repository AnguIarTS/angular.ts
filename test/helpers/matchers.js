...
beforeEach(() => {
  ...
  function MinErrMatcher(isNot, namespace, code, content, wording) {
    ...
    const contentRegex = angular.isUndefined(content) || jasmine.isA_('RegExp', content) ?
    ...
    function test(exception) {
      ...
      const contentMatches = angular.isUndefined(contentRegex) || contentRegex.test(exceptionMessage);
  ...
  jasmine.addMatchers({
    ...
    toBeShown() {
      ...
      function generateCompare(isNot) {
        return function(actual) {
          const message = valueFn(`Expected element ${  isNot ? '' : 'not '  }to have 'ng-hide' class`);
    ...
    toEqualOneOf(util) {
      return {
        compare(actual) {
          ...
          return {
            pass: expectedArgs.some((expected) => util.equals(actual, expected, [DOMTester]))
          };
        }
      };
    },
