...
describe('ngHref', () => {
  ...
  it('should interpolate the expression and bind to href', inject(($compile, $rootScope) => {
  ...
  it('should bind href and merge with other attrs', inject(($rootScope, $compile) => {
  ...
  it('should bind href even if no interpolation', inject(($rootScope, $compile) => {
    element = $compile('<a ng-href="http://server"></a>')($rootScope);
    $rootScope.$digest();
    expect(element.attr('href')).toEqual('http://server');
  }));

  it('should not set the href if ng-href is empty', inject(($rootScope, $compile) => {
    $rootScope.url = null;
  ...
  it('should remove the href if ng-href changes to empty', inject(($rootScope, $compile) => {
  ...
  if (msie || /\bEdge\/1[2-7]\.[\d.]+\b/.test(window.navigator.userAgent)) {
    ...
    it('should throw error if ng-href contains a non-escaped percent symbol', inject(($rootScope, $compile) => {
      expect(() => {
  ...
  it('should bind numbers', inject(($rootScope, $compile) => {
  ...
  it('should bind and sanitize the result of a (custom) toString() function', inject(($rootScope, $compile) => {
    $rootScope.value = {};
  ...
  if (isDefined(window.SVGElement)) {
    describe('SVGAElement', () => {
      it('should interpolate the expression and bind to xlink:href', inject(($compile, $rootScope) => {
      ...
      it('should bind xlink:href even if no interpolation', inject(($rootScope, $compile) => {
