...
describe('ngOn* event binding', () => {
  it('should add event listener of specified name', inject(($compile, $rootScope) => {
    $rootScope.name = 'Misko';
    const element = $compile('<span ng-on-foo="name = name + 3"></span>')($rootScope);
    element.triggerHandler('foo');
    expect($rootScope.name).toBe('Misko3');
  }));

  it('should use angular.element(x).on() API to add listener', inject(($compile, $rootScope) => {
  ...
  it('should allow access to the $event object', inject(($rootScope, $compile) => {
    const element = $compile('<span ng-on-foo="e = $event"></span>')($rootScope);
    element.triggerHandler('foo');
    expect($rootScope.e.target).toBe(element[0]);
  }));

  it('should call the listener synchronously', inject(($compile, $rootScope) => {
    const element = $compile('<span ng-on-foo="fooEvent()"></span>')($rootScope);
  ...
  it('should support multiple events on a single element', inject(($compile, $rootScope) => {
    const element = $compile('<span ng-on-foo="fooEvent()" ng-on-bar="barEvent()"></span>')($rootScope);
    $rootScope.fooEvent = jasmine.createSpy('fooEvent');
  ...
  it('should work with different prefixes', inject(($rootScope, $compile) => {
    ...
    const element = $compile('<span ng:on:test="cb(1)" ng-On-test2="cb(2)" ng_On_test3="cb(3)"></span>')($rootScope);
  ...
  it('should work if they are prefixed with x- or data- and different prefixes', inject(($rootScope, $compile) => {
    const cb = $rootScope.cb = jasmine.createSpy('ng-on cb');
    const element = $compile('<span data-ng-on-test2="cb(2)" x-ng-on-test3="cb(3)" data-ng:on-test4="cb(4)" ' +
  ...
  it('should work independently of attributes with the same name', inject(($rootScope, $compile) => {
    const element = $compile('<span ng-on-asdf="cb()" asdf="foo" />')($rootScope);
    const cb = $rootScope.cb = jasmine.createSpy('ng-on cb');
    $rootScope.$digest();
  ...
  it('should work independently of (ng-)attributes with the same name', inject(($rootScope, $compile) => {
    const element = $compile('<span ng-on-asdf="cb()" ng-attr-asdf="foo" />')($rootScope);
    const cb = $rootScope.cb = jasmine.createSpy('ng-on cb');
    $rootScope.$digest();
    element.triggerHandler('asdf');
    expect(cb).toHaveBeenCalled();
  ...
  it('should work independently of properties with the same name', inject(($rootScope, $compile) => {
    const element = $compile('<span ng-on-asdf="cb()" ng-prop-asdf="123" />')($rootScope);
    const cb = $rootScope.cb = jasmine.createSpy('ng-on cb');
    $rootScope.$digest();
    element.triggerHandler('asdf');
    expect(cb).toHaveBeenCalled();
  ...
  it('should correctly bind to kebab-cased event names', inject(($compile, $rootScope) => {
  ...
  it('should correctly bind to camelCased event names', inject(($compile, $rootScope) => {
