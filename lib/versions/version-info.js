...
let NO_REMOTE_REQUESTS = !!process.env['NG1_BUILD_NO_REMOTE_VERSION_REQUESTS'];
let versionSource = NO_REMOTE_REQUESTS ? 'local' : 'remote';

let currentPackage, previousVersions, cdnVersion;
...
let getPackage = function() {
  ...
  while (!fs.existsSync(path.join(packageFolder, 'package.json'))) {
    let parent = path.dirname(packageFolder);
 ...
 */
let getGitRepoInfo = function() {
  let GITURL_REGEX = /^https:\/\/github.com\/([^/]+)\/(.+).git$/;
  let match = GITURL_REGEX.exec(currentPackage.repository.url);
  let git = {
    owner: match[1],
    repo: match[2]
  };
 ...
 */
let getCodeName = function(tagName) {
  let gitCatOutput = shell.exec('git cat-file -p ' + tagName, {silent:true}).stdout;
  let tagMessage = gitCatOutput.match(/^.*codename.*$/mg)[0];
  let codeName = tagMessage && tagMessage.match(/codename\((.*)\)/)[1];
  if (!codeName) {
 ...
 */
function getBuild() {
  let hash = shell.exec('git rev-parse --short HEAD', {silent: true}).stdout.replace('\n', '');
  return 'sha.' + hash;
}
 ...
 */
let getTaggedVersion = function() {
  let gitTagResult = shell.exec('git describe --exact-match', {silent:true});

  if (gitTagResult.code === 0) {
    let tag = gitTagResult.stdout.trim();
 ...
 */
let getPreviousVersions =  function() {
  ...
  let repo_url = currentPackage.repository.url;
  let query = NO_REMOTE_REQUESTS ? 'git tag' : 'git ls-remote --tags ' + repo_url;
  let tagResults = shell.exec(query, {silent: true});
    ...
    .reduce(function(cdnVersion, version) {
      if (!cdnVersion) {
        if (NO_REMOTE_REQUESTS) {
          ...
          let cdnResult = shell.exec('curl http://ajax.googleapis.com/ajax/libs/angularjs/' + version + '/angular.min.js ' +
                    '--head --write-out "%{http_code}" -silent',
                                      {silent: true});
          if (cdnResult.code === 0) {
            // --write-out appends its content to the general request response, so extract it
            let statusCode = cdnResult.stdout.split('\n').pop().trim();
            if (statusCode === '200') {
              cdnVersion = version;
            }
          }
 ...
 */
let getSnapshotVersion = function() {
  ...
  let ciBuild = process.env.CIRCLE_BUILD_NUM || process.env.BUILD_NUMBER;
