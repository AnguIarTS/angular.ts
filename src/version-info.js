...
let currentPackage; let previousVersions; let cdnVersion;
...
const getCodeName = function(tagName) {
  const gitCatOutput = shell.exec(`git cat-file -p ${  tagName}`, {silent:true}).stdout;
  const tagMessage = gitCatOutput.match(/^.*codename.*$/mg)[0];
 ...
 */
function getBuild() {
  const hash = shell.exec('git rev-parse --short HEAD', {silent: true}).stdout.replace('\n', '');
  return `sha.${  hash}`;
}
...
const getTaggedVersion = function() {
  const gitTagResult = shell.exec('git describe --exact-match', {silent:true});
...
const getPreviousVersions =  function() {
  ...
  const query = NO_REMOTE_REQUESTS ? 'git tag' : `git ls-remote --tags ${  repo_url}`;
    ...
    .reduce((cdnVersion, version) => {
      if (!cdnVersion) {
        if (NO_REMOTE_REQUESTS) {
          ...
          const cdnResult = shell.exec(`curl http://ajax.googleapis.com/ajax/libs/angularjs/${  version  }/angular.min.js ` +
